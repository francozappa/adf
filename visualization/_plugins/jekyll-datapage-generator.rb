...
#
# Copyright (c) 2025 Tropic Square All rights reserved.
# Modified for ADF Threat Model Generator
#
#  Changelog: added 'key' parameter to make the key string accessible (not only value in the key-value pair)
#
...
module Jekyll
  ...
  class DataPage < Page
    ...
    def initialize(site, base, index_files, dir, page_data_prefix, data, key, name, name_expr, title, title_expr, template, extension, debug)
      @site = site
      @base = base
      ...
      if data
        data.each do |data_spec|
          ...
          if not site.layouts.key? template
            ...
            records.zip(keys).each do |record,key|
              site.pages << DataPage.new(site, site.source, index_files_for_this_data, dir, page_data_prefix, record, key, name, name_expr, title, title_expr, template, data_spec['extension'], debug)
            end
          end
        end
      end
